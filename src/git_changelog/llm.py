    ...
    def _post(
        ...
        try:
            ...
            raise LLMError(
                ...
                "Raise CHANGELOG_TIMEOUT if the model is still loading."
            ) from exc
...
def build_client(provider: str | None = None, model: str | None = None) -> LLMClient:
    """Build the client selected by --provider or CHANGELOG_PROVIDER. Default is Ollama."""
    selected = (provider or os.environ.get("CHANGELOG_PROVIDER") or "ollama").strip().lower()
    ...
    if selected == "openai":
        ...
        if "api.openai.com" in base_url and not api_key:
            raise LLMError(
                ...
                "Set it, or leave CHANGELOG_PROVIDER=ollama to generate the changelog locally."
            )
    ...
    raise LLMError("CHANGELOG_PROVIDER must be 'ollama' or 'openai'.")


def _timeout_seconds() -> float:
    raw = os.environ.get("CHANGELOG_TIMEOUT", str(DEFAULT_TIMEOUT_SECONDS))
    try:
        value = float(raw)
    except ValueError as exc:
        raise LLMError("CHANGELOG_TIMEOUT must be a number of seconds.") from exc
    if value <= 0:
        raise LLMError("CHANGELOG_TIMEOUT must be positive.")
    return value
